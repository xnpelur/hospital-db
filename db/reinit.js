...
async function down() {
    ...
    try {
        ...
        const oldUsers = await client.query(
            `SELECT a.rolname FROM pg_roles a INNER JOIN pg_auth_members b ON a.oid = b.member INNER JOIN pg_roles c ON b.roleid = c.oid WHERE c.rolname = 'patient' OR c.rolname = 'doctor';`
        );
